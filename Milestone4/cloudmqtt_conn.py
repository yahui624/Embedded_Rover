...
def on_message(client, userdata, message):
    ...
    required_fields = ["encoderVal_1", "encoderVal_2", "encoderVal_3"]
    ...
    if "EncoderCheckSum" in transmitting_JSON:  # for the client side statistics
         pub_client_Statistics(transmitting_JSON)

    if "encoderVal_1" in transmitting_JSON:  # Means the data comes from the mqtt client
        ...
        if msg_Valid:  # Means this message is VALID
            ...
            statistics_data['Seq_Num']  =  transmitting_JSON['sequence_no'] # update the sequence number
...
def publish_Statistics():
    ...
    '''
    published_msg = client.publish("/Server/Statistics", encode_object(statistics_data))
    client.publish("/Server/Statistics", "publishing to server/Statistics")
